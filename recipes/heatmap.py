...
def plot_heatmap(exp):
    fig, ax = plt.subplots()
    im = ax.imshow(exp, aspect='auto')
    ax.axvline(x=1, color='black')
    canvas = fig.canvas
    canvas.mpl_connect('scroll_event', scroll_callback)
    canvas.mpl_connect('key_press_event', key_press_callback)
    canvas.mpl_connect('button_press_event', button_press_callback)

def _button_press_callback(event):
    rx = int(round(event.xdata))
    ry = int(round(event.ydata))
    print("row: %d    column: %d" % (rx, ry))

def _key_press_callback(event, base_scale=2., offset=1):
    ax = event.inaxes
...
def _scroll_callback(event, base_scale=2.):
    ax = event.inaxes
